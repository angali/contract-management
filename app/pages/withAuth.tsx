...
const getUserByToken = async (role: string, context: any) => {
  ...
  if (token) {
    try {
      const response = await axios.get(
        `${BACKEND_API_URL}/${role === "admin" ? "admin" : "user"}`,
        {
          headers: {
...
// withAuth.js (ref: https://github.com/vercel/next.js/discussions/10925)
export function withAuthServerSideProps(
  ...
  return async (context: any) => {
    const { user,  token } = await getUserByToken(role, context);
